...
function EditPost() {
    ...
    return (
        <Card className="mx-auto max-w-4xl rounded-lg p-4 shadow-md">
            <h2 className="mb-6 text-center text-3xl font-semibold">Edit Lecture</h2>
            <form onSubmit={submitChanges} className="space-y-6">
                ...
                <div>
                    ...
                    <Input id="image" {...register("image", { required: "Image URL is required" })} placeholder="Image URL..."/>
                    {errors.image && <ErrorMessage error={errors.image.message} />}
                </div>
                <div>
                    <Label htmlFor="lectureURL">Lecture URL</Label>
                    <Input id="lectureURL" {...register("lectureURL", { required: "Lecture URL is required" })} placeholder="Lecture URL..."/>
                    {errors.lectureURL && <ErrorMessage error={errors.lectureURL.message} />}
                </div>
                <Button type="submit" className="w-full">Update Lecture</Button>
            </form>
        </Card>
    );
}
